...
def before_request():
    ...
    Check if user is logged in, and in that case sets the last_seen to the current time.
    """
...
def user(username):
    ...
    form = EmptyForm()  # when following/unfollowing users, for redirecting
...
@login_required
def edit_profile():
    form = EditProfileForm()  # staviti argument current_user.username ako vratim ono sa __init__
    if form.validate_on_submit():
        current_user.username = form.username.data
        current_user.about_me = form.about_me.data
        db.session.commit()
        flash(_('Your profile has been updated.'))
        return redirect(url_for('edit_profile'))  # TODO Videti da li treba ovde da ga vraca
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('You no longer follow %(username)s!', username=username))
        return redirect(url_for('main.user', username=username))
    else:
        return redirect(url_for('main.index'))
